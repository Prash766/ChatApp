...
const ChatWindow = () => {
  ...
  useEffect(() => {
    if (isUserTyping?.isTyping && isAtBottom()) {
      if (endOfMessagesRef.current) {
        endOfMessagesRef.current.scrollIntoView({ behavior: "smooth" });
      }
    }
      ...
      >
        ...
        {showScrollButton && (
          <button
            ...
            className="fixed bottom-24 right-8 bg-gray-800 dark:bg-gray-700 
                     text-white rounded-full p-3 shadow-lg hover:bg-gray-700 
                     dark:hover:bg-gray-600 transition-all duration-300 z-10"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`p-2 rounded-full transition-all duration-300 ${
              message ||
              imageUrls.length > 0 ||videos.length > 0 ||
              files.length > 0
                ? "bg-blue-500 hover:bg-blue-600 cursor-pointer text-white"
                : "bg-blue-300 cursor-default text-gray-200"
            }`}
